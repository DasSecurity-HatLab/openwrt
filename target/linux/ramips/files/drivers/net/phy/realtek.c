...
/* 
	...
	Modified for RTL8211FS auto mode switch 
*/
#define RTL8211FS_FIBER_ESR			0x0F
#define RTL8211FS_FIBER_MODE		0xC000
...
static int rtl8211f_config_init(struct phy_device *phydev)
{
	...
	phydev->priv = kzalloc(sizeof(struct rtl8211f_priv), GFP_KERNEL);
	if (!phydev->priv)
		return -ENOMEM;
...
static int rtl8211f_mode(struct phy_device *phydev)
{
	...
	val = phy_read(phydev, RTL8211FS_FIBER_ESR) & RTL8211FS_FIBER_MODE;
...
static int rtl8211f_config_aneg(struct phy_device *phydev)
{
	...
	if (rtl8211f_mode(phydev) == RTL8211FS_MODE_FIBER) {
		...
		priv->oldmode = RTL8211FS_MODE_FIBER;
		return genphy_c37_config_aneg(phydev);
	}

	dev_info(&phydev->mdio.dev, "Copper Mode");

	priv->oldmode = RTL8211F_MODE_COPPER;
...
static int rtl8211f_read_status(struct phy_device *phydev)
{
	...
	if(rtl8211f_mode(phydev) != priv->oldmode) {
